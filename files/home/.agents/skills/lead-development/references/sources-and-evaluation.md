...
- [OpenAI subagent documentation](https://developers.openai.com/codex/subagents/)
  describes model/effort selection and orchestration. Available identifiers and
  spawn fields must still be verified against the current tool. A skill can
  request model routing; it cannot make an unavailable model executable.
